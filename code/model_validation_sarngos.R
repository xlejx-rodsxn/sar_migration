...
nrow(df_min_A[which(df_min_A$date <= ymd("2014-10-01")),])

# Variables and cross-validation set-up

target <- df_min_A$crossings_CMR[1:39]
X_vars <- data.frame(lapply(df_min_A[1:39,-c(1,2,3)], function(x) scale(x)))
...
state.spec_default <- impact_sarngos$model$bsts.model$state.specification
...
post.period <- c(40, 113)
post.period.response <- target_y[post.period[1] : post.period[2]]

target_y[post.period[1] : post.period[2]] <- NA
sdy_y <- sd(target_y)
X_vars <- data.frame(lapply(df_min_A[,-c(1,2)], function(x) scale(x)))
X_vars <- X_vars[,-which(colSums(is.na(X_vars))>0)]
covars_df <- as.matrix(X_vars)

model_sarngos_state.spec_locallineartrend <- bsts(target_y ~ as.matrix(covars_df),
                                                      state.specification=state.spec_locallineartrend,
                                                      niter=2500, max.flips=100,
                                                      seed=270488)
model_sarngos_state.spec_locallevel <- bsts(target_y ~ as.matrix(covars_df),
                                                state.specification=state.spec_locallevel,
                                                niter=2500, max.flips=100,
                                                seed=270488)
model_sarngos_state.state.spec_semilocallineartrend <- bsts(target_y ~ as.matrix(covars_df),
                                                                state.specification=state.spec_semilocallineartrend,
                                                                niter=2500, max.flips=100,
                                                                seed=270488)
model_sarngos_state.spec_autoar <- bsts(target_y ~ as.matrix(covars_df),
                                            state.specification=state.spec_autoar,
                                            niter=2500, max.flips=100,
                                            seed=270488)

model_sarngos_state.spec_combined1 <- bsts(target_y ~ as.matrix(covars_df),
                                               state.specification=state.spec_combined1,
                                               niter=2500, max.flips=100,
                                               seed=270488)
model_sarngos_state.spec_combined2 <- bsts(target_y ~ as.matrix(covars_df),
                                               state.specification=state.spec_combined2,
                                               niter=2500, max.flips=100,
                                               seed=270488)
model_sarngos_state.spec_combined3 <- bsts(target_y ~ as.matrix(covars_df),
                                               state.specification=state.spec_combined3,
                                               niter=2500, max.flips=100,
                                               seed=270488)
model_sarngos_state.spec_combined4 <- bsts(target_y ~ as.matrix(covars_df),
                                               state.specification=state.spec_combined4,
                                               niter=2500, max.flips=100,
                                               seed=270488)

impact_sarngos_state.spec_locallineartrend <- CausalImpact(bsts.model = model_sarngos_state.spec_locallineartrend, post.period.response = post.period.response)
impact_sarngos_state.spec_locallevel <- CausalImpact(bsts.model = model_sarngos_state.spec_locallevel, post.period.response = post.period.response)
impact_sarngos_state.state.spec_semilocallineartrend <- CausalImpact(bsts.model = model_sarngos_state.state.spec_semilocallineartrend, post.period.response = post.period.response)

impact_sarngos_state.spec_combined1 <- CausalImpact(bsts.model = model_sarngos_state.spec_combined1, post.period.response = post.period.response)
impact_sarngos_state.spec_combined2 <- CausalImpact(bsts.model = model_sarngos_state.spec_combined2, post.period.response = post.period.response)
impact_sarngos_state.spec_combined3 <- CausalImpact(bsts.model = model_sarngos_state.spec_combined3, post.period.response = post.period.response)
impact_sarngos_state.spec_combined4 <- CausalImpact(bsts.model = model_sarngos_state.spec_combined4, post.period.response = post.period.response)

compare_mod <- as_tibble(CompareBstsModels(list("LLT" = impact_sarngos_state.spec_locallineartrend$model$bsts.model,
                                                "LL" = impact_sarngos_state.spec_locallevel$model$bsts.model,
                                                "SLLT" = impact_sarngos_state.state.spec_semilocallineartrend$model$bsts.model,
                                                "LLT+AR" = impact_sarngos_state.spec_combined1$model$bsts.model,
                                                "SLLT+AR" = impact_sarngos_state.spec_combined2$model$bsts.model,
                                                "LL+AR" = impact_sarngos_state.spec_combined3$model$bsts.model,
                                                "LL + SLLT + AR" = impact_sarngos_state.spec_combined4$model$bsts.model,
                                                "Default" = impact_sarngos$model$bsts.model),
                                           main = "Model Comparison:  NGOs effects"))
...
effs_sarngos <- bind_rows(data.frame(model = "LLT", impact_sarngos_state.spec_locallineartrend$summary)[1,],
                  data.frame(model = "LL", impact_sarngos_state.spec_locallevel$summary)[1,],
                  data.frame(model = "SLT", impact_sarngos_state.state.spec_semilocallineartrend$summary)[1,],
                  data.frame(model = "LLT + AR", impact_sarngos_state.spec_combined1$summary)[1,],
                  data.frame(model = "SLT + AR", impact_sarngos_state.spec_combined2$summary)[1,],
