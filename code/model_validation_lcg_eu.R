...
nrow(df_min_A[which(df_min_A$date <= ymd("2017-01-01")),])

target <- df_min_A$crossings_CMR[1:66]
X_vars <- data.frame(lapply(df_min_A[1:66,-c(1,2,3)], function(x) scale(x)))
...
res_df_covar_ <- cbind(name = c("state.spec_locallineartrend",
                                "state.spec_semilocallineartrend",
                                "state.spec_locallevel",
                                "state.spec_combined1",
                                "state.spec_combined2",
...
res_df_covar_ %>% arrange(mean_mase)
...
table_sarlibya <- res_df_covar_ %>% arrange(mean_mase)
colnames(table_sarlibya) <- c("Models","RMSE","MAPE","MASE")
table_sarlibya
saveRDS(table_sarlibya, file="D:/Postdoc/DEZIM/DeZIM/SAR project/manuscript/table_sarlibya.RDS")

# Estimate the causal effect for all models

target_y <- df_min_A$crossings_CMR[order(df_min_A$date)]
post.period <- c(67, 113)
...
model_sarlibya_state.spec_locallineartrend <- bsts(target_y ~ as.matrix(covars_df),
                                                      state.specification=state.spec_locallineartrend,
                                                      niter=2500, max.flips=100,
                                                      seed=270488)
model_sarlibya_state.spec_locallevel <- bsts(target_y ~ as.matrix(covars_df),
                                                state.specification=state.spec_locallevel,
                                                niter=2500, max.flips=100,
                                                seed=270488)
model_sarlibya_state.state.spec_semilocallineartrend <- bsts(target_y ~ as.matrix(covars_df),
                                                                state.specification=state.spec_semilocallineartrend,
                                                                niter=2500, max.flips=100,
                                                                seed=270488)
model_sarlibya_state.spec_autoar <- bsts(target_y ~ as.matrix(covars_df),
                                            state.specification=state.spec_autoar,
                                            niter=2500, max.flips=100,
                                            seed=270488)

model_sarlibya_state.spec_combined1 <- bsts(target_y ~ as.matrix(covars_df),
                                               state.specification=state.spec_combined1,
                                               niter=2500, max.flips=100,
                                               seed=270488)
model_sarlibya_state.spec_combined2 <- bsts(target_y ~ as.matrix(covars_df),
                                               state.specification=state.spec_combined2,
                                               niter=2500, max.flips=100,
                                               seed=270488)
model_sarlibya_state.spec_combined3 <- bsts(target_y ~ as.matrix(covars_df),
                                               state.specification=state.spec_combined3,
                                               niter=2500, max.flips=100,
                                               seed=270488)
model_sarlibya_state.spec_combined4 <- bsts(target_y ~ as.matrix(covars_df),
                                               state.specification=state.spec_combined4,
                                               niter=2500, max.flips=100,
                                               seed=270488)

impact_sarlibya_state.spec_locallineartrend <- CausalImpact(bsts.model = model_sarlibya_state.spec_locallineartrend, post.period.response = post.period.response)
impact_sarlibya_state.spec_locallevel <- CausalImpact(bsts.model = model_sarlibya_state.spec_locallevel, post.period.response = post.period.response)
impact_sarlibya_state.state.spec_semilocallineartrend <- CausalImpact(bsts.model = model_sarlibya_state.state.spec_semilocallineartrend, post.period.response = post.period.response)

impact_sarlibya_state.spec_combined1 <- CausalImpact(bsts.model = model_sarlibya_state.spec_combined1, post.period.response = post.period.response)
impact_sarlibya_state.spec_combined2 <- CausalImpact(bsts.model = model_sarlibya_state.spec_combined2, post.period.response = post.period.response)
impact_sarlibya_state.spec_combined3 <- CausalImpact(bsts.model = model_sarlibya_state.spec_combined3, post.period.response = post.period.response)
impact_sarlibya_state.spec_combined4 <- CausalImpact(bsts.model = model_sarlibya_state.spec_combined4, post.period.response = post.period.response)

compare_mod <- as_tibble(CompareBstsModels(list("LLT" = impact_sarlibya_state.spec_locallineartrend$model$bsts.model,
                                                "LL" = impact_sarlibya_state.spec_locallevel$model$bsts.model,
                                                "SLLT" = impact_sarlibya_state.state.spec_semilocallineartrend$model$bsts.model,
                                                "LLT+AR" = impact_sarlibya_state.spec_combined1$model$bsts.model,
                                                "SLLT+AR" = impact_sarlibya_state.spec_combined2$model$bsts.model,
                                                "LL+AR" = impact_sarlibya_state.spec_combined3$model$bsts.model,
                                                "LL + SLLT + AR" = impact_sarlibya_state.spec_combined4$model$bsts.model,
                                                "Default" = impact_sarlibya$model$bsts.model),
                                           main = "Model Comparison: EU and Libya cooperation effects"))
...
compare_mod_long_sarlibya <- compare_mod %>% 
  pivot_longer(1:113)

saveRDS(compare_mod_long_sarlibya, file="D:/Postdoc/DEZIM/DeZIM/SAR project/manuscript/compare_mod_long_sarlibya.RDS")

effs_sarlibya <- bind_rows(data.frame(model = "LLT", impact_sarlibya_state.spec_locallineartrend$summary)[1,],
                  data.frame(model = "LL", impact_sarlibya_state.spec_locallevel$summary)[1,],
                  data.frame(model = "SLT", impact_sarlibya_state.state.spec_semilocallineartrend$summary)[1,],
                  data.frame(model = "LLT + AR", impact_sarlibya_state.spec_combined1$summary)[1,],
                  data.frame(model = "SLT + AR", impact_sarlibya_state.spec_combined2$summary)[1,],
                  data.frame(model = "LL + AR", impact_sarlibya_state.spec_combined3$summary)[1,],
                  data.frame(model = "LL + SLT + AR", impact_sarlibya_state.spec_combined4$summary)[1,],
                  data.frame(model = "Def.: LL", impact_sarlibya$summary)[1,])
saveRDS(effs_sarlibya, file="D:/Postdoc/DEZIM/DeZIM/SAR project/manuscript/effs_sarlibya.RDS")

ggplot(effs_sarlibya, aes(x=model, y=AbsEffect, ymin=AbsEffect.lower, ymax=AbsEffect.upper)) + 
  geom_point() + 
  geom_errorbar() +
...
# FINAL PLOTS
parameter_a <- par(mfrow=c(1,3)) #set up the plotting space
parameter_a <- AcfDist(impact_marenostrum$model$posterior.samples, main="A. Mare Nostrum")
parameter_a <- AcfDist(impact_sarngos$model$posterior.samples, main = "B. NGOs")
parameter_a <- AcfDist(impact_sarlibya$model$posterior.samples, main = "C. EU and Libya cooperation")

parameter_b <- par(mfrow=c(1,3)) #set up the plotting space
parameter_b <- qqdist(impact_marenostrum$model$posterior.samples, main="A. Mare Nostrum")
parameter_b <- qqdist(impact_sarngos$model$posterior.samples, main = "B. NGOs")
parameter_b <- qqdist(impact_sarlibya$model$posterior.samples, main = "C. EU and Libya cooperation")
